...
export class NetworkStorageController {
  ...
  private extractXuid(xuidPart: any): string {
	if (typeof xuidPart !== 'string' || xuidPart.trim() === '') {
      throw new Error('Invalid xuidPart');
    }
    const xuid = xuidPart.match(/^xuid\((\d+)\)$/);

    if (!xuid) {
      throw new Error('Invalid XUID');
    }

    return BigInt(xuid[1]).toString(16).toUpperCase().padStart(16, '0');
  }

  
  private extractTitleId(titleIdPart: any): string {
    const titleId = Number(titleIdPart);

    if (!Number.isSafeInteger(titleId) || titleId < 0) {
      throw new Error('Invalid title id');
    }

    return BigInt(titleIdPart).toString(16).toUpperCase().padStart(8, '0');
  }
  ...
  private storageDir(xuid: string, titleId: string) {
	if (typeof xuid !== "string" || xuid.trim() === "") {
      throw new Error("Invalid xuid");
    }
	if (typeof titleId !== "string" || titleId.trim() === "") {
      throw new Error("Invalid title id");
    }
    return join(this.storageRoot, xuid, titleId);
  }
  ...
  async getTitleInfo(@Req() req: Request, @Res() res: Response) {
    const xuid = this.extractXuid(req.params.xuidPart);
    const titleId = this.extractTitleId(req.params.titleId);
    try {
